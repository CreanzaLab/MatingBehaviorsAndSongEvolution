#Unified BayesTraits Analysis plot code
#5/24/18 - added SongParam to Low, Medium/High etc in arrow plots, made rate = 0 arrows gray, made "Song" into "SongRep" in plots
#6/7/2018 - started making for loop in transitionplots to doa  transition plot averaging all runs, not segments
#setwd("~/Creanza Lab/Mating Systems/BayesTraits Discrete threshold tests")
...
with(df, plot(songcontvec, LRpval, pch=20, 
              main = paste(MateSongParams, " Removed: ",familysplit,"; Original threshold range: ", round(min(alldfvals),4)," - ",round(max(alldfvals),3)," (",length(alldfvals),")", sep = ""), xlim = c(min(songvals)-0.1*min(songvals),max(songvals)+0.1*max(songvals)), log = "x", xlab = "", ylab="", cex.main = 0.7
))
